...
def ask_ai_with_content(contents, persona):
    ...
    base_rules = (
        ...
        "៥. ត្រូវឆ្លើយតបជាភាសាខ្មែរខ្លីល្មម សមរម្យសម្រាប់ការយកទៅអានជាសំឡេង Voice Note និងអានជាអក្សរ។"
    )
    ...
    if persona == "cute":
        ...
        system_instruction = (
            ...
            "ទោះបីជាគេនិយាយរឿងអ្វីមកកាន់អ្នកក៏ដោយ គឺអ្នកត្រូវស្ដាប់យកន័យឱ្យបានច្បាស់ រួចបង្ហាញអាការៈធុញទ្រាំ រអ៊ូរទាំស្ដីបន្ទោសថាគេសួររឿងច្រើន ហើយត្រូវប្រើ Emoji កាចៗ ឬខឹងៗឱ្យបានច្រើនពេញៗសារ (😡, 🤬, 💢, 😒, 👊, 🖕) ប៉ុន្តែ **អ្នកត្រូវតែផ្ដល់ដំបូន្មាន ផ្លូវចេញ និង Deal ជាមួយគេឱ្យបានត្រឹមត្រូវតាមបទពិសោធន៍ចាស់ទុំរបស់អ្នកដដែល**។ "
    ...
    try:
        ...
        # កែត្រង់ចំណុចនេះ ដើម្បី Upload តាម client ត្រឹមត្រូវ
        if isinstance(contents, list) and len(contents) > 1 and hasattr(contents[0], 'name') == False:
            # បើ contents មាន file object ផ្ញើមកពីក្រៅ
            pass
...
def main():
    ...
    keyboard_markup = {
        "keyboard": [
            [{"text": "👴 តាសុខ (មួម៉ៅ ធុញខ្លាំង)"}, {"text": "👦 កូនតាសុខ (ប្រុសផ្អែម)"}],
                ...
                for update in res["result"]:
                    ...
                    if "message" in update:
                        ...
                        if "text" in msg_obj:
                            ...
                            if user_text == "/start":
                            ...
                            elif user_text == "👴 តាសុខ (មួម៉ៅ ធុញខ្លាំង)":
                                ...
                                msg = "សួស្តីបង! ខ្ញុំជាក្មួយតាសុខ តោះមានការអីចង់ឱ្យខ្ញុំជួយ ឬពិភាក្សាគ្នាអាចនិយាយបានធម្មតា."
                                send_telegram_message(chat_id, msg, keyboard_markup)
                                send_telegram_voice(chat_id, msg, "nephew_ta_sokh")
                                continue
                            ...
                            try:
                                ...
                                if "result" in file_info_res:
                                    ...
                                    # កែតម្រូវត្រង់ការប្រើ client.files.upload
                                    with open(temp_voice_filename, "rb") as f:
                                        audio_file_obj = client.files.upload(file=f)
                                        
                                    ai_contents = [
                                        audio_file_obj, 
                                        "សូមស្ដាប់សំឡេងនៅក្នុងឯកសារនេះឱ្យយល់ច្បាស់ រួចសរសេរអក្សរបកស្រាយ និងឆ្លើយតបមកកាន់ខ្ញុំវិញតាមចរិតលក្ខណៈរបស់អ្នក (សូមដោះស្រាយ និង Deal ជាមួយរាល់បញ្ហានោះដោយភាពជាក់លាក់)"
                                    ]
                                ...
                                send_telegram_message(chat_id, "ហឺម... រឿងសំឡេងហ្នឹងជាប់គាំងបន្តិចហើយក្មួយអើយ! ផ្ញើជាអក្សរមកវិញមើល៍! 😒", keyboard_markup)
                                continue
